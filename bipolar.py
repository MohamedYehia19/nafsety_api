...
data_dir = r"D:\model\Bipolar_model"

#  Train/Test
train_file = f"{data_dir}/MDQ_train.csv"
test_file  = f"{data_dir}/MDQ_test.csv"
...
print(f"MDQ Accuracy: {accuracy_score(y_test_enc, y_pred)*100:.2f}%")
print(classification_report(y_test_enc, y_pred, zero_division=0))

joblib.dump(clf, f"{data_dir}/MDQ_model.pkl")
joblib.dump(le, f"{data_dir}/MDQ_encoder.pkl")
json.dump({int(i): c for i, c in enumerate(le.classes_)},
          open(f"{data_dir}/MDQ_map.json", "w"),
          indent=2)

print("Saved MDQ model files in", data_dir)
